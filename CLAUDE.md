...
Releases are cut per artifact, never all at once — see `release.md` for the full checklist. Tagging alone builds nothing: `.github/workflows/release-windows.yml` (NSIS) and `release-web.yml` (portable `diveplay-web.html`) are `workflow_dispatch` only and take the tag as an input. The Linux AppImage is built locally with `./scripts/build-appimage.sh` (Docker, `ubuntu:22.04`) and uploaded by hand with `gh release upload`; no `.deb` is produced by CI.

The AppImage bundles its own FFmpeg and WebKitGTK, so it must curate what it ships: `usr/lib/dphost/` holds libraries that would otherwise shadow ABI-coupled host ones (the bundled wayland stack broke the *host's* Mesa EGL loader, which killed the WebProcess on any Mesa ≥ 25 machine), and `scripts/dp-glprobe.c` + `scripts/dp-run.sh` pick and then verify a GL tier at startup. If a `.deb` is built by hand instead, it declares `ffmpeg` as a system dependency and `get_sidecar_path` falls back to `/usr/bin/ffmpeg`, so the `binaries/ffmpeg` and `binaries/ffprobe` files in `src-tauri/binaries/` stay dev-only.
